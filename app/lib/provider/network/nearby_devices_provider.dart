...
/// It will override any existing device with the same fingerprint.
class RegisterDeviceAction
  ...
  Future<NearbyDevicesState> reduce() async {
    ...
    if (favoriteDevice != null) {
      // Update existing favorite: always sync latest IP/port so future scans reach the device,
      // and update alias if the user hasn't set a custom one.
      final updatedFavorite = favoriteDevice.copyWith(
        ip: device.ip ?? favoriteDevice.ip,
        port: device.port,
        alias: favoriteDevice.customAlias ? favoriteDevice.alias : device.alias,
      );
      await external(
        notifier._favoriteService,
      ).dispatchAsync(UpdateFavoriteAction(updatedFavorite));
    } else {
      await Future.microtask(() {});
    }
    return state.copyWith(
      devices: {...state.devices}
        ..update(device.fingerprint, (_) => device, ifAbsent: () => device),
    );
  }
}
